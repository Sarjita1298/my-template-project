...
<div class="container">
    ...
    <form action="{{ route('blogs.update', $blog->id) }}" method="POST" enctype="multipart/form-data" class="card p-4 shadow-sm rounded-3">
        @csrf
        @method('PUT')

        <div class="mb-3">
            <label for="blog_title" class="form-label">Blog Title <span class="text-danger">*</span></label>
            <input type="text" name="blog_title" id="blog_title" class="form-control" value="{{ old('blog_title', $blog->blog_title) }}" required>
        </div>
